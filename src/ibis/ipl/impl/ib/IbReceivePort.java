/* $Id: IbReceivePort.java 14244 2012-01-31 08:42:05Z ceriel $ */
...
class IbReceivePort extends ReceivePort implements IbProtocol {
                ...
                if (lazy_connectionhandler_thread) {
                    ...
                    for (;;) {
                        if (logger.isDebugEnabled()) {
                            logger.debug("lazy handler sleeping " + interval
                                    + " ms.");
                        }
                        synchronized (this) {
                            // Wait on this handler. We cannot use the port for
                            // this,
                            // since that one wakes up when finish() is called
                            // on a message.
                            try {
                                wait(interval);
                            } catch (Throwable e) {
        ...
        void reader(boolean fromHandlerThread) throws IOException {
            ...
            while (in != null) {
                ...
                case CLOSE_ALL_CONNECTIONS:
                    ...
                    close(null);
                    if (lazy_connectionhandler_thread && !fromHandlerThread) {
                        // Wake up the connection handler thread so that it can
                        // die.
                        synchronized (this) {
                            notifyAll();
                        }
                ...
                case CLOSE_ONE_CONNECTION:
                    ...
                    if (ident.equals(identifier)) {
                        ...
                        //
                        // This is here to make sure the close is processed
                        // before a new
                        // connections can be made (by the same sendport).
                        // Without this ack,
                        // an application that uses a single sendport that
                        // connects/disconnects
                        // for each message may get an
                        // 'AlreadyConnectedException', because the
                        // connect overtakes the disconnect...
                        //
                        // Unfortunately, it also causes a deadlock in 1-to-1
                        // explict receive
                        // applications -- J
                        //
                        ...
                        in = null;
                        if (logger.isDebugEnabled()) {
                            logger.debug(port.name + ": connection with "
                                    + origin + " closing");
                        }
    ...
    public synchronized void closePort(long timeout) {
        ...
        if (lazy_connectionhandler_thread && conns.length > 0) {
            // Wakeup connection handler thread, otherwise this may take a
            // second ...
